...
const startChat = asyncHandler(async (req, res) => {
    ...
    // Determine sender type (User or Doctor)
    const sender = await User.findById(senderId) || await Doctor.findById(senderId);
    const receiver = await User.findById(receiverId) || await Doctor.findById(receiverId);
    ...
    if (!chat) {
        chat = new Chat({
            chatRoomId,
            users: [
                { userId: senderId, userType: sender instanceof Doctor ? 'Doctor' : 'User' },
                { userId: receiverId, userType: receiver instanceof Doctor ? 'Doctor' : 'User' }
            ]
        });
        await chat.save();
    }
...
export const sendMessage = asyncHandler(async (req, res) => {
    ...
    const newMessage = new Message({
        ...
        sender: {
            userId: senderId,
            userType: sender instanceof Doctor ? 'Doctor' : 'User'
        },
        message
    });
